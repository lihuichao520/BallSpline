...
//jqq 2013.12.17
template <class T, int N>
void NurbsCurve<T,N>::normalizeKnot()
{
  T maxt, mint;
...
/*!
  \brief Computes the curvature of the  curve at parameter \a u
...
*/
template <class T, int N>
T NurbsCurve<T, N>::curvature( const T& u )
{
	Vector<Point_nD<T,N>> ders;
	ders.resize(3);
	Point_nD<T,N>& der1 = ders[1];
	Point_nD<T,N>& der2 = ders[2];
	T k=0;
	deriveAt(u,2,ders);
	if (N==2){
		cout << "der1: " << der1 << "der2: " << der2;
		k = abs(der1.x()*der2.y()-der2.x()*der1.y());
		k = k / pow( der1.x() * der1.x() + der1.y()*der1.y(), 1.5 );
		return k;
	}
	else if (N==3){
		k = pow(der2.z()*der1.y()-der2.y()*der1.z(),2);
		k += pow(der2.x()*der1.z()-der2.z()*der1.x(),2);
		k += pow(der2.y()*der1.x()-der2.x()*der1.y(),2);
		k = sqrt(k);
		k = k/pow(pow(der1.x(),2)+pow(der1.y(),2)+pow(der1.z(),2),1.5);
		return k;
	}
...
/*!
  \brief Determines the feasible knot span index

  Determines the feasible knot span for any parameter u.deg_  
  functions. The span is the index \a k for which the parameter 
  \a u is valid in the [u_k,u_{k+1}] range.
  
  \param u  the parametric value
  \return the span index at \a u.
  \warning \a the the feasible spans index will be stored in member FS
  \author Apango Cheng 
  \date 21 May 2012
  \modified 21 May, 2012 (Apango Cheng)
*/
template <class T, int N>
void NurbsCurve<T,N>::findFeasibleSpans() {
...
/*!
  \brief Generate the Coefficient Matrix for polynomial style express of curve

  Generate the Coefficient Matrix for polynomial style express of curve
...
template <class T, int N>
void NurbsCurve<T, N>::generatePolyCoefVectors()
{
	findFeasibleSpans();
	Cxs.resize(FS.n());
	Cys.resize(FS.n());
	...
	{
		//cout << "  i = " << i << "  " << "  span = " << FS[i] << std::endl;
		//cout << "* * * * * * * * * * * * * *";
		generatePolyCoefVectorForOneSpan(FS[i],Cxs[i],Cys[i],Czs[i],Cws[i]);
	}
}

/*!
  \brief Generate the Coefficient Matrix for one span

  Generate the Coefficient Matrix for one span
...
template <class T, int N>
void NurbsCurve<T, N>::generatePolyCoefVectorForOneSpan( int span, Vector<T>& Cx, Vector<T>& Cy, Vector<T>& Cz, Vector<T>& Cw ){
	
	Matrix<T> matA = Matrix<T>( degree()+1, degree()+1 );
	Matrix<T> matU = Matrix<T>( degree()+1, degree()+1 );
	Matrix<T> matN = Matrix<T>( degree()+1, degree()+1 );

	Vector<T> vecU = Vector<T>( degree()+1 );
	Vector<T> vecN = Vector<T>( degree()+1 );
	...
	//store the scattered u_j in [u_i,u_i+1]
	for(int k=0;k<degree()+1;k++){
		vecU[k]=U[span]+k*(U[span+1]-U[span])/(degree()+1);  
	}

	//cout << "vecU:" << std::endl << vecU << std::endl;
	//store the U matrix
	//made by the scattered u_j and their powers
	//for order 0 to degree
	for (int i=0;i<degree()+1;i++){
		for (int j=0;j<degree()+1;j++){
			matU[i][j]=pow(vecU[i],j);
		}
	}
	//cout << "matU" << std::endl << matU << std::endl;
	//store the N vector, basisfuns
	for (int i=0;i<degree()+1;i++){
		basisFuns(vecU[i],span,vecN);
		for (int j=0;j<degree()+1;j++){
			matN[i][j] = vecN[j];
		}
	}
	//cout << "matN:" << std::endl << matN << std::endl; 
	//cout << "invU:" << std::endl << inverse(matU) << std::endl;
	//calculate the coefficient matrix for basis functions
	matA = inverse(matU) * matN;
	//cout << "matA:" << std::endl << matA << std::endl; 
	//calculate the coefficients for polynomial style NURBS
	for (int k=0;k<degree()+1;k++){
		for (int j=0;j<degree()+1;j++){
			Cx[k] += matA[k][j]*P[span-degree()+j].x();
			Cy[k] += matA[k][j]*P[span-degree()+j].y();
			Cz[k] += matA[k][j]*P[span-degree()+j].z();
			Cw[k] += matA[k][j]*P[span-degree()+j].w();
		}
	}
	//cout << P << std::endl;
	//cout << "Cx:" << std::endl << Cx << std::endl;
	//cout << "Cy:" << std::endl << Cy << std::endl;
	//cout << "Cz:" << std::endl << Cz << std::endl;
	//cout << "Cw:" << std::endl << Cw << std::endl;
...
BasicList<Point_nD<T,N> > NurbsCurve<T,N>::tesselate(T tolerance,BasicList<T> *uk) const {
  ...
  if(ca.n()==1){
    ...
    int n = (int)floor(sqr2*norm(maxD)/tolerance) ;
