...
namespace PLib {
  ...
  */
  template <class T, int N>
	...
	{ return deg_ ; }
	  int CtrlNum() { return P.n();}           //jqq 2013.12.17
	...
	{ return U[i]; }
	  const Vector<int>& feasibleSpans() const //!< a reference to the vector of feasible spans
	{ return FS; }
	  const Vector<LUMatrix<T>> & polyCoefMatrices() const //!< a reference to the vector of polynomial coefficient matrices
	{ return PCM; }
      // basic functions
	  ...
	  // Polynomial Style Expression Function
	  void generatePolyCoefVectors();
      void generatePolyCoefVectorForOneSpan( int span, Vector<T>& Cx, Vector<T>& Cy, Vector<T>& Cz, Vector<T>& Cw );
    ...
    protected:
	  ...
	  Vector<int> FS;  // the feasible spans
	  Vector<Vector<T>> Cxs; // the polynomial coefficient matrices for x coordinate
	  Vector<Vector<T>> Cys; // the polynomial coefficient matrices for y coordinate
	  Vector<Vector<T>> Czs; // the polynomial coefficient matrices for x coordinate
	  Vector<Vector<T>> Cws; // the polynomial coefficient matrices for y coordinate
