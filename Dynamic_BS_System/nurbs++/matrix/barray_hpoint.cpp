...
=============================================================================*/
#ifndef barray_hpoint_h__
#define barray_hpoint_h__
...
void resizeBasicArrayHPoint(BasicArray<HPoint_nD<T,D> >& a, int nsize){
  ...
  /* * * * * * * * * * * * * * * * * * 
   The type of the data member in HPoint_nD has been modified from T * to T data[D+1]
   no need to allocate memory for the data member again.
  * * * * * * * * * * * * * * * * * */

  //T* data = new T[nsize*(D+1)];
  //
  //for(k=nsize-1;k>=0;--k){
	 // copyFromPointerToArray( xn[k].data, data+ k*(D+1) );
